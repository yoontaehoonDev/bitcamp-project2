...
public class MemberTable implements DataTable {
	...
	public void service(Request request, Response response) throws Exception {
		...
		case "member/inputMember":
			temp = request.getDataList().get(0);
			name = inputMember(temp);

			if(name.equals(null)) {
				throw new Exception("해당 이름이 없습니다.");
			}
			break;
		case "member/inputMembers":
			temp = request.getDataList().get(0);
			name = inputMembers(temp);

			if(name == null) {
				throw new Exception("해당 이름이 없습니다.");
			}
			break;
		case "member/selectByKeyword":
	...
	public String inputMember(String promptTitle) {
		while (true) {
			...
			if (!findByName(name).equals(null)) {
				return name;
			}
