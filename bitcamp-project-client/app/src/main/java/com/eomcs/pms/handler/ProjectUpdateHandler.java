...
public class ProjectUpdateHandler implements Command {
	...
	public void service(DataInputStream in, DataOutputStream out) {
		...
		try {System.out.println("[프로젝트 변경]");
		...
		String owner = Prompt.inputString(String.format("만든이(%s)?(취소: 빈 문자열) ", fields[5]));
		if (owner == null) {
			System.out.println("프로젝트 변경을 취소합니다.");
			return;
		}
		String members = Prompt.inputString(String.format("팀원(%s)?(취소: 빈 문자열) ", fields[6]));
